...
from homeassistant.core import CALLBACK_TYPE, HomeAssistant, SupportsResponse, callback
...
class IBeaconCoordinator:
    ...
    def __init__(
        ...
        self._whitelisted_uuids: list[str] = []
    ...
    def _async_update_ibeacon_with_unique_address(
        ...
        ) and uuid not in self._whitelisted_uuids:
            _LOGGER.info(f"Reject new iBeacon uuid:{uuid} is not in whitelist {self._whitelisted_uuids} and has no name")
            return
    ...
    async def async_start(self) -> None:
        ...
        @callback
        def handle_whitelist_add(call):
            uuid = call.data.get(ATTR_UUID)
            self._whitelisted_uuids.append(uuid)
            _LOGGER.info(f"Added {uuid} to Whitelist.")
            self._last_ibeacon_advertisement_by_unique_id.clear()
            self._last_seen_by_group_id.clear()
            self._transient_seen_count.clear()
